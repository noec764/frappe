...
from frappe.integrations.doctype.nextcloud_settings.nextcloud_filesync.diff_engine.utils_time import (
...
class TestNCFetcher(NextcloudTester):
	...
	def test_filter_works_dotfiles(self):
		...
		self.assertTrue(all(("no" not in x.path) or (x.path == self.common.root) for x in all_files))
