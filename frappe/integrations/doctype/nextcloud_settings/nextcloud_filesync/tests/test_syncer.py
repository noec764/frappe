...
from frappe.integrations.doctype.nextcloud_settings.nextcloud_filesync.diff_engine.tests._tester import (
	NextcloudTester,
	using_local_files,
	using_remote_files,
)
...
class TestSyncer(NextcloudTester):
	...
	def test_missing_remote_parent(self):
		...
		children = frappe.get_all("File", {"folder": "Home/test_missing_remote_parent/A"})

		for f in children:
			doc = frappe.get_doc("File", f)
			print(f"cofudir({str(doc)}, False)")
