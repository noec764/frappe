...
frappe.ui.form.on("Calendar View", {
	...
	reference_doctype: function (frm) {
		...
		frappe.model.with_doctype(reference_doctype, () => {
			...
			const value_field_options = meta.fields
			...
			const color_options = meta.fields
				.filter((df) => ["Color"].includes(df.fieldtype))
				.map((df) => df.fieldname);

			const check_options = meta.fields
				.filter((df) => ["Check"].includes(df.fieldtype))
				.map((df) => df.fieldname);

			const select_options = meta.fields
				.filter((df) => ["Select"].includes(df.fieldtype))
				.map((df) => df.fieldname);
