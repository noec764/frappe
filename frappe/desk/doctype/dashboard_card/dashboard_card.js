...
frappe.ui.form.on('Dashboard Card', {
	...
	show_filters: function(frm) {
		if (frm.card_filters && frm.card_filters.length) {
			frm.trigger('render_filters_table');
		} else {
			if (frm.doc.card_type==='Custom') {
				if (frm.doc.source) {
	...
	setup_card(frm) {
		if (frm.doc.card_type==='Custom' && frm.doc.source) {
			frappe.xcall('frappe.desk.doctype.dashboard_card_source.dashboard_card_source.get_config', {name: frm.doc.source})
				.then(config => {
					frappe.dom.eval(config);
