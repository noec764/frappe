			...
			<div class="text-medium flex justify-between">
				<span :title="file.name">
					<a :href="file.doc.file_url" v-if="file.doc" target="_blank">
						<i v-if="file.doc.is_private" class="uil uil-lock-alt text-info"></i>
						<i v-else class="uil uil-lock-open-alt text-warning"></i>
						{{ file.name | file_name }}
					</a>
					<span v-else>
						<span class="cursor-pointer" @click="$emit('toggle_private')" :title="__('Toggle Public/Private')">
							<i v-if="file.private" class="uil uil-lock-alt text-info"></i>
							<i v-else class="uil uil-lock-open-alt text-warning"></i>
						</span>
						{{ file.name | file_name }}
					</span>
