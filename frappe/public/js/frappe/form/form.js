...
frappe.ui.form.Form = class FrappeForm {
	...
	show_tour(on_finish) {
		this.tour.init({ on_finish }).then(() => this.tour.start());
	}
	...
	amend_doc() {
		...
		frappe
			...
			.then((is_amended) => {
				if (is_amended) {
					frappe.throw(
						__("This document is already amended, you cannot amend it again")
					);
				}
	...
	add_custom_button(label, fn, group) {
		...
		if (
			group &&
			(group.indexOf("fa fa-") !== -1 ||
				group.indexOf("fas fa-") !== -1 ||
				group.indexOf("uil uil-") !== -1)
		)
			group = null;
	...
	/**
	 * @note Do not use `set_df_property` for table fields, instead consider using `frm.fields_dict[fieldname].grid.update_docfield_property`
	 */
