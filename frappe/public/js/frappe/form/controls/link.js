...
frappe.ui.form.ControlLink = class ControlLink extends frappe.ui.form.ControlData {
	...
	setup_awesomeplete() {
		...
		this.$input.on("focus", function () {
			if (!(frappe.boot.translated_search_doctypes && frappe.boot.translated_search_doctypes.includes(me.df.options))) {
		...
		this.$input.keydown((e) => {
			let BACKSPACE = 8;
			if (e.keyCode === BACKSPACE && !(frappe.boot.translated_search_doctypes && frappe.boot.translated_search_doctypes.includes(me.df.options))) {
				me.show_untranslated();
			}
		});
	}
