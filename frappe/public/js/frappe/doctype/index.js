...
frappe.model.DocTypeController = class DocTypeController extends frappe.ui.form.Controller {
	setup() {
		...
		this.setup_docfield_configuration();
	}
	...
	setup_docfield_configuration() {
		const heuristics = {
			is_number: (df) =>
				["Int", "Float", "Currency", "Percent"].some((type) =>
					df.fieldtype.includes(type)
				),
		...
		const field_dt = this.frm.doctype === "DocType" ? "DocField" : "Customize Form Field";
		frappe.ui.form.on(field_dt, {
			form_render: (frm, cdt, cdn) => {
				const row_frm = cur_frm.get_field("fields").grid.open_grid_row;
