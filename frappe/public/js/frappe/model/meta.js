...
$.extend(frappe.meta, {
	...
	get_fields_to_check_permissions: function(doctype) {
		...
		fields = fields.concat({label: __("ID"), fieldname: name, options: doctype});
		return fields;
	},
	...
	get_field_precision: function(df, doc) {
		...
		if (df && df.precision) {
			...
			precision = cint(frappe.defaults.get_default("currency_precision")) || cint(frappe.boot.sysdefaults.float_precision);
			if(!precision) {
