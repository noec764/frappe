...
def format_time(
	time_string=None, format_string: str | None = None, lang: str | None = None
) -> str:
	...
	* HH:mm
	...
	try:
		formatted_time = babel.dates.format_time(
			time_, format_string, locale=(lang or "").replace("-", "_")
		)
	except UnknownLocaleError:
	...
	* mm-dd-yyyy HH:mm
	...
	try:
		formatted_datetime = babel.dates.format_datetime(
			datetime, format_string, locale=(lang or "").replace("-", "_")
		)
	except UnknownLocaleError:
...
def global_date_format(date, format="long", lang=None):
	...
	date = getdate(date)
	formatted_date = babel.dates.format_date(
		date, locale=(lang or "en").replace("-", "_"), format=format
	)
	return formatted_date
...
def money_in_words(
	...
	if main == "0" and fraction in ["00", "000"]:
		if symbol_on_right:
			out = _("Zero") + " " + _(currency_title, context="Currency")
		else:
			out = _(currency_title, context="Currency") + " " + _("Zero")
	...
	# 0.XX
		...
		if symbol_on_right:
			out = _(in_words(main, in_million).title()) + " " + _(currency_title, context="Currency")
		else:
			out = _(currency_title, context="Currency") + " " + _(in_words(main, in_million).title())
