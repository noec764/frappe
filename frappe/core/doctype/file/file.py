...
class File(Document):
	...
	def after_rename(self, *args, **kwargs):
		for successor in self.get_successors():
			setup_folder_path(successor, self.name)  # noqa
	...
	def handle_is_private_changed(self):
		...
		update_existing_file_docs(self)  # noqa
	...
	def generate_content_hash(self):
		...
		try:
			...
			with open(file_path, "rb") as f:
				self.content_hash = get_content_hash(f.read())  # noqa
		except OSError:
	...
	def make_thumbnail(
		...
		try:
			if self.file_url.startswith(("/files", "/private/files")):
				image, filename, extn = get_local_image(self.file_url)  # noqa
			else:
				image, filename, extn = get_web_image(self.file_url)  # noqa
	...
	def get_content(self) -> bytes:
		...
		if self.get("content"):
			...
			if self.decode:
				self._content = decode_file_content(self._content)  # noqa
				self.decode = False
	...
	def save_file(
		...
		self.content_hash = get_content_hash(self._content)  # noqa
		...
		if not file_exists:
			if not overwrite:
				self.file_name = generate_file_name(  # noqa
					name=self.file_name,
	...
	def delete_file_from_filesystem(self, only_thumbnail=False):
		...
		if only_thumbnail:
			delete_file(self.thumbnail_url)  # noqa
		else:
			delete_file(self.file_url)  # noqa
			delete_file(self.thumbnail_url)  # noqa
