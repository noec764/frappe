...
class Communication(Document, CommunicationEmailMixin):
	...
	if TYPE_CHECKING:
		...
		not_added_to_reference_timeline: DF.Check
	...
	for email in email_addrs:
		...
		if not contact_name and email and auto_create_contact:
			...
			try:
				...
				contact.log_error(_("Unable to add contact"))
...
def update_parent_document_on_communication(doc):
	...
	if status_field:
		...
		# In Dokos: Don't reopen all documents automatically as it may be misleading for users (eg. Lead)
		...
		if len(communications):
			...
			for i in range(len(communications)):
				if (
					...
					response_time = round(
						time_diff_in_seconds(communications[i].creation, communications[i - 1].creation),
						2,
					)
					if response_time > 0:
