...
from frappe.desk.doctype.notification_settings.notification_settings import (
    create_notification_settings, toggle_notifications)
from frappe.desk.notifications import clear_notifications
from frappe.model.document import Document
from frappe.query_builder import DocType
from frappe.rate_limiter import rate_limit
from frappe.utils import (cint, escape_html, flt, format_datetime,
                          get_formatted_email, get_time_zone, has_gravatar,
                          now_datetime, today)
...
class User(Document):
	...
	def set_roles_and_modules_based_on_user_type(self):
		...
		if user_type_doc.role:
			...
			if user_linked_with_permission_on_doctype(user_type_doc, self.name):
				...
				frappe.msgprint(
					_("Role has been set as per the user type {0}").format(_(self.user_type)),
					alert=True,
				)
	...
	def get_fullname(self):
		...
		return (
			(self.first_name or "") + (self.first_name and " " or "") + (self.last_name or "")
		)
	...
	def send_welcome_mail_to_user(self):
		...
		email_template = frappe.db.get_value(
			"System Settings", None, "template_for_welcome_email"
		)
		if email_template:
	...
	def after_rename(self, old_name, new_name, merge=False):
		...
		if frappe.db.exists("Notification Settings", old_name):
			frappe.rename_doc(
				"Notification Settings", old_name, new_name, force=True, show_alert=False
			)
...
def handle_password_test_fail(result):
	suggestions = (
		result["feedback"]["suggestions"][0] if result["feedback"]["suggestions"] else ""
	)
...
def get_role_profile(role_profile):
	roles = frappe.get_doc("Role Profile", role_profile)
	return roles.roles
...
def create_contact(user, ignore_links=False, ignore_mandatory=False):
	...
	contact_name = get_contact_name(user.email) or frappe.db.get_value(
		"Contact", dict(user=user.name)
	)
	if not contact_name:
