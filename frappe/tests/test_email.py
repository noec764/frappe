...
class TestEmail(unittest.TestCase):
		...
		for part in email_obj.walk():
			...
			if content:
				...
				frappe.local.flags.signed_query_string = re.search(
					r"(?<=/api/method/frappe.email.queue.unsubscribe\?).*(?=" + eol + ")",
					content.decode(),
				).group(0)
	...
	def test_image_parsing(self):
		...
		self.assertTrue(
			re.search("""<img[^>]*src=["']/private/files/rtco1[^>]*>""", communication.content)
		)
		self.assertTrue(
			re.search("""<img[^>]*src=["']/private/files/rtco2[^>]*>""", communication.content)
		)
