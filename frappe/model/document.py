...
class Document(BaseDocument):
	...
	def has_permission(self, permtype="read", verbose=False):
		...
		return frappe.permissions.has_permission(
			self.doctype, permtype, self, verbose=verbose
		)
	...
	def insert(
		...
		self.set_new_name(
			draft_name=getattr(self.meta, "name_after_submit", False)
			and not self._action == "submit",
			set_name=set_name,
	...
	def set_new_name(
		self, draft_name=False, force=False, set_name=None, set_child_names=True
	):
		...
		if draft_name and getattr(self.meta, "name_after_submit"):
			frappe.db.set_value(
				self.doctype, self.name, "_draft_name", self.name, update_modified=False
			)
	...
	def add_submitted_record(self):
		if getattr(self.meta, "is_submittable"):
			...
			frappe.db.set_value(
				self.doctype, self.name, "_submitted", self._submitted, update_modified=False
			)
			frappe.db.set_value(
				self.doctype, self.name, "_submitted_by", self._submitted_by, update_modified=False
			)
	...
	def _validate_non_negative(self):
		...
		for df in self.meta.get(
			"fields",
			{"non_negative": ("=", 1), "fieldtype": ("in", ["Int", "Float", "Currency"])},
		):
