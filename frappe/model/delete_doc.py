...
def delete_doc(
	...
	for name in names or []:
		...
		if doctype == "DocType":
			...
			if (
				hasattr(frappe.get_meta(doctype), "is_sealed")
				and frappe.get_meta(doctype).is_sealed
				and doc.docstatus != 0
			):
...
def update_naming_series(doc):
	if doc.meta.autoname:
		...
		elif doc.meta.autoname.split(":", 1)[0] not in (
			"Prompt",
			"field",
			"hash",
			"autoincrement",
...
def raise_link_exists_exception(doc, reference_doctype, reference_docname, row=""):
	doc_link = get_link_to_form(doc.doctype, doc.name, _(doc.name))
	reference_link = get_link_to_form(reference_doctype, reference_docname, _(reference_docname))
