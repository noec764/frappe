...
from frappe.utils.data import nowdate, sbool
...
def rename_dynamic_links(doctype: str, old: str, new: str):
	...
	for df in get_dynamic_link_map().get(doctype, []):
		...
		if is_single:
			refdoc = (
				frappe.db.get_values(df.parent, fieldname=[df.options, df.fieldname], as_dict=True) or {}
			)
...
def add_renaming_log(doctype, old, new):
	"""
	Specific to Dokos
	Creates a log of all renaming actions
	"""
	frappe.get_doc(
		{
