...
class DBTable:
	...
	def get_columns_from_docfields(self):
		...
		if not self.meta.get("istable"):
			...
			if self.meta.get("track_print"):
				fields.append({"fieldname": "_printed", "fieldtype": "Datetime"})

			# add _submitted and _submitted_by columns if is_submittable
			if self.meta.get("is_submittable"):
				fields.append({"fieldname": "_submitted", "fieldtype": "Datetime"})
				fields.append({"fieldname": "_submitted_by", "fieldtype": "Data"})

			# add _seal column if is_sealed
			if self.meta.get("is_sealed"):
				fields.append({"fieldname": "_seal", "fieldtype": "Data"})

			# add _draft_name column if name_after_submit
			if self.meta.get("name_after_submit"):
				fields.append({"fieldname": "_draft_name", "fieldtype": "Data"})
...
class DbColumn:
	...
	def build_for_alter_table(self, current_def):
		...
		if (self.unique and not current_def["unique"]) and column_type not in (
			...
			self.table.add_unique.append(self)
		elif (current_def["unique"] and not self.unique) and column_type not in (
			"text",
			"longtext",
		):
