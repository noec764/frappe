...
""")
		...
		if self.set_property_after_alert:
			...
			try:
				if allow_update and not doc.flags.in_notification_update:
					...
					# Reload doc to avoid timestamp mismatch error
					doc.reload()
	...
	def get_attachment(self, doc):
		...
		if (doc.docstatus == 0 and not print_settings.allow_print_for_draft) or \
			...
			frappe.throw(_("""Not allowed to attach {0} document, please enable Allow Print For {0} in Print Settings""").format(_(status)),
...
def get_context(doc):
	return {"doc": doc, "nowdate": nowdate, "frappe": frappe._dict(
